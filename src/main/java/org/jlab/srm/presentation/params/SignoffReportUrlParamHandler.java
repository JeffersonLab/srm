...
public class SignoffReportUrlParamHandler implements UrlParamHandler<SignoffReportParams> {
  ...
  public SignoffReportUrlParamHandler(
  ...
  public SignoffReportParams materialize() {
    ...
    HttpSession session = request.getSession(true);
    BigInteger[] destinationIdArray =
        (BigInteger[]) session.getAttribute("signoffReportDestinationId[]");
    BigInteger[] categoryIdArray = (BigInteger[]) session.getAttribute("signoffReportCategoryId[]");
    ...
    String[] componentNameArray = (String[]) session.getAttribute("signoffReportComponentName[]");
   ...
   */
  @Override
  public void redirect(HttpServletResponse response, SignoffReportParams params)
      throws IOException {
