...
public class Signoff extends HttpServlet {
  ...
  private static final Logger LOGGER = Logger.getLogger(Signoff.class.getName());
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      systemFirst = ParamUtil.convertAndValidateYNBoolean(request, "systemFirst", false);
    } catch (Exception e) {
      ...
      if (groupResponsibilityList != null && !groupResponsibilityList.isEmpty()) {
        params.setGroupId(
            groupResponsibilityList
                .get(groupResponsibilityList.size() - 1)
                .getGroup()
                .getGroupId());
      }
    }
    ...
    if (selectedSystem != null && params.getGroupId() != null) {
      ...
      if (!request.isUserInRole("srm-admin")
          && groupResponsibilityForSelected != null
          && groupResponsibilityForSelected.isChecklistRequired()
          && !groupResponsibilityForSelected.isPublished()) {
        // Don't grab the list of components since it will be a waste, but set to non-null to avoid
        // presentation thinking user didn't select criteria
        ...
        if (componentList != null) {
          ...
          List<SignoffStandIn> signoffList =
              signoffFacade.findStandInListByComponentList(componentList);
          signoffMap = signoffFacade.createComponentGroupToSignoffStandInMap(signoffList);
        }
      }
    }
    ...
    if (componentList != null) {
      ...
      String filters =
          FilterSelectionMessage.getSignoffScreenMessage(
              selectedGroup,
              null,
              selectedSystem,
    ...
    request.setAttribute("groupResponsibilityForSelected", groupResponsibilityForSelected);
    ...
    getServletConfig()
        .getServletContext()
        .getRequestDispatcher("/WEB-INF/views/signoff.jsp")
