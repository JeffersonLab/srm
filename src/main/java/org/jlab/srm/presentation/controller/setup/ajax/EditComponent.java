...
@WebServlet(
    name = "EditComponent",
    urlPatterns = {"/setup/ajax/edit-component"})
public class EditComponent extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      } else if (rootCause instanceof XAException) {
        /*We're likely dealing with deferred constraint*/
        XAException txException = (XAException) rootCause;

        if (txException.getMessage().contains("GROUP_SIGNOFF_FK1")) {
          errorReason =
              "Cannot reassign component to a new system because the component has signoffs.  Select the force option if a loss of signoffs / signoff history is acceptable.";
        } else {
