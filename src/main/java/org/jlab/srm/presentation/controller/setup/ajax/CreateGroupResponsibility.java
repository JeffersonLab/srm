 ...
 */
@WebServlet(
    name = "CreateGroupResponsibility",
    urlPatterns = {"/setup/ajax/create-group-responsibility"})
public class CreateGroupResponsibility extends HttpServlet {

  private static final Logger logger = Logger.getLogger(CreateGroupResponsibility.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      BigInteger order = ParamConverter.convertBigInteger(request, "order");

      logger.log(
          Level.FINE,
          "SystemId: {0}; GroupId: {1}, Order: {2}",
          new Object[] {systemId, groupId, order});

      groupResponsibilityId =
          responsibilityFacade.createNew(systemId, groupId, checklistRequired, order);
