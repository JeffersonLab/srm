...
public final class FilterSelectionMessage {
    ...
    public static String getTrendReportMessage(Date start, Date end) {
        ...
        return "Daily Ready Signoff Count from " + formatter.format(start) + " to "
                + formatter.format(end);
    }

    public static List<Map.Entry<String, Object>> getOverallStatusFootnoteList(
            List<BeamDestination> selectedDestinationList,
            Category selectedCategory, SystemEntity selectedSystem, Region selectedRegion,
        ...
        if (selectedDestinationList != null && !selectedDestinationList.isEmpty()) {
            ...
            Map.Entry<String, Object> entry = new AbstractMap.SimpleEntry<>("Beam Destination", selectedDestinationList);

            footnoteList.add(entry);
        }

        if (selectedCategory != null) {
            Map.Entry<String, Object> entry = new AbstractMap.SimpleEntry<>("Category", selectedCategory.getName());
        ...
        if (selectedRegion != null) {
            Map.Entry<String, Object> entry = new AbstractMap.SimpleEntry<>("Region", selectedRegion.getName());

            footnoteList.add(entry);
        }

        if (selectedGroup != null) {
            Map.Entry<String, Object> entry = new AbstractMap.SimpleEntry<>("Group", selectedGroup.getName());
    ...
    public static String getMessage(List<BeamDestination> destinationList, Category category,
                                    SystemEntity system, Region region, ResponsibleGroup group, List<Status> statusList,
                                    Boolean masked, Boolean unpowered, String componentName,
                                    String reason, DataSource source) {
    ...
    public static String getSavedSignoffMessage(SavedSignoffType type, SystemEntity system,
    ...
    public static String getActivityScreenMessage(List<BeamDestination> destinationList,
                                                  Category category,
                                                  SystemEntity system,
                                                  Region region, ResponsibleGroup group, String username, String componentName,
                                                  Status status, SignoffChangeType change, Date start, Date end) {
        List<String> filters = new ArrayList<>();
    ...
    public static String getSignoffScreenMessage(ResponsibleGroup group, Category category,
                                                 SystemEntity system, List<BeamDestination> destinationList, List<Region> regionList,
                                                 List<Status> statusList, Boolean readyTurn, Boolean masked, String componentName,
                                                 Date minLastModifiedDate, Date maxLastModifiedDate) {
        ...
        if (componentName
                != null && !componentName.isEmpty()) {
            filters.add("Name \"" + componentName + "\"");
        }

        if (minLastModifiedDate
                != null) {
            filters.add("Min Last Modified Date \"" + dateFormat.format(
                    minLastModifiedDate) + "\"");
        }

        if (maxLastModifiedDate
                != null) {
            filters.add("Max Last Modified Date \"" + dateFormat.format(
                    maxLastModifiedDate) + "\"");
        }
