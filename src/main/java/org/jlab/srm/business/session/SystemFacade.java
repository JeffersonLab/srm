...
public class SystemFacade extends AbstractFacade<SystemEntity> {
  ...
  public List<SystemEntity> findAllHco() {
    Query q =
        em.createNativeQuery(
            "select * from system where system_id in (select system_id from system_application where application_id = 1) order by weight asc, name asc",
            SystemEntity.class);
  ...
  public List<SystemEntity> findByGroup(BigInteger groupId) {
    ...
    Query q =
        em.createNativeQuery(
            "select * from system where system_id in (select system_id from system_application where application_id = 1) and system_id in (select system_id from group_responsibility where group_id = :groupId) order by weight asc, name asc",
            SystemEntity.class);
  ...
  public List<SystemEntity> findByBeamDestination(BigInteger destinationId) {
    String query =
        "select * from system where system_id in (select distinct system_id from component a, component_beam_destination b where a.component_id = b.component_id and beam_destination_id = :destinationId)";
  ...
  public List<SystemEntity> findWithCategory(
    ...
    // If searching by component, or by system_id, or for all systems (no filter), or by category,
    // but without recursion
    if (componentId != null
        || systemId != null
        || (componentId == null && systemId == null && categoryIdArray == null)
        || (categoryIdArray != null && !recurse)) {
      ...
      systemList =
          findByComponentCategoryAndSystem(componentId, categoryId, systemId, applicationId);
    } else {
  ...
  public List<SystemEntity> findWithCategory(
    ...
    if (componentId != null
        || systemId != null
        || (componentId == null && systemId == null && categoryId == null)
        || (categoryId != null && !recurse)) {
      systemList =
          findByComponentCategoryAndSystem(componentId, categoryId, systemId, applicationId);
    } else {
      // componentId == null && systemId == null && categoryId != null && recurse == true
  ...
  public List<SystemEntity> gatherDescendents(Category category, BigInteger applicationId) {
    ...
    if (category.getSystemList() == null || category.getSystemList().isEmpty()) {
      ...
      for (SystemEntity system : category.getSystemList()) {
        if (applicationId == null
            || system.getApplicationList().contains(Application.FROM_ID(applicationId))) {
