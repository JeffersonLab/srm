...
public class MaskingRequestFacade extends AbstractFacade<MaskingRequest> {
    ...
    @PermitAll
    public void createRequest(List<BigInteger> componentIdList, String reason, Date expirationDate) throws
            ...
            for (BigInteger componentId : componentIdList) {
                ...
                if (c != null) {
                    ...
                    if (isRequestAlreadyPending(c)) {
                        throw new UserFriendlyException("Component mask request already pending: " + c.getName());
                    }
    ...
    @PermitAll
    public List<MaskingRequest> find(BigInteger[] destinationIdArray, BigInteger categoryId,
                                     BigInteger systemId, BigInteger regionId, BigInteger groupId, String reason,
                                     MaskingRequestStatus status, int offset,
                                     int maxPerPage) {
    ...
    @PermitAll
    public Long count(BigInteger componentId, BigInteger[] destinationIdArray, BigInteger categoryId, BigInteger systemId,
                      BigInteger regionId, BigInteger groupId, String reason, MaskingRequestStatus status) {
        CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
 ...
 /*statusIdArray = IOUtil.removeNullValues(statusIdArray, BigInteger.class);
    ...
    @PermitAll
    public void acceptMaskRequest(BigInteger maskRequestId, String reason, Date expiration) throws
            UserFriendlyException {
        if (maskRequestId == null) {
            throw new UserFriendlyException("masking request ID must not be empty");
        }
        ...
        //    throw new UserFriendlyException("Component " + component.getName() + " is already masked");
        //}
        ...
        componentFacade.editMasked(component.getComponentId(), true, reason, expiration, Status.MASKED.getStatusId().intValue());
    }

    @PermitAll
    public boolean isRequestAlreadyPending(Component c) {
        long count = count(c.getComponentId(), null, null, null, null, null, null, MaskingRequestStatus.PENDING);
