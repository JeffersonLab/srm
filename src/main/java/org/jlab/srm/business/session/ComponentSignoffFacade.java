...
public class ComponentSignoffFacade {
  ...
  public List<StatusCount> getStatusCount(
    ...
    if (regionId != null) {
      w =
          "a.component_id in (select component_id from component where region_id = "
              + regionId
              + ")";
      whereList.add(w);
    }
    ...
    // Query q = em.createNativeQuery("select a.status_id, count(b.status_id) from status a,
    // component_signoff b where a.status_id = b.status_id (+) group by a.status_id");
  ...
  @SuppressWarnings("unchecked")
  @PermitAll
  public List<GroupStatusCount> getGroupStatusCount(
      BigInteger[] destinationIdArray,
      BigInteger categoryId,
      BigInteger systemId,
      BigInteger regionId) {
    ...
    String select =
        "select c.group_id, c.name, c.goal_percent, "
    ...
    String query =
        "select * from ("
            ...
            + " "
            + from
            + ") where ready > 0 or checked > 0 or not_ready > 0 order by name asc";
    ...
    for (Object[] row : results) {
      ...
      GroupStatusCount count =
          new GroupStatusCount(groupId, name, goalPercent, ready, checked, notReady, total);
  ...
  @SuppressWarnings("unchecked")
  @PermitAll
  public List<SignoffReportRecord> filterSignoffReportRecordList(
      BigInteger[] destinationIdArray,
      BigInteger categoryId,
      BigInteger systemId,
      BigInteger regionId,
    ...
    w = "a.component_id = b.component_id and a.group_id = c.group_id and a.system_id = d.system_id";
    whereList.add(w);
  ...
  @SuppressWarnings("unchecked")
  @PermitAll
  public Long countSignoffReportRecordList(
      BigInteger[] destinationIdArray,
      BigInteger categoryId,
      BigInteger systemId,
      BigInteger regionId,
