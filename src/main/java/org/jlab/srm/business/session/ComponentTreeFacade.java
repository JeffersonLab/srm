...
public class ComponentTreeFacade {
  ...
  private static final Logger logger = Logger.getLogger(ComponentTreeFacade.class.getName());
  ...
  /**
   * In order to filter by category and system we first check if specific system is specified and
   * return that as the one and only system. Otherwise if category is specified then grab all system
   * IDs which are underneath that category.
   *
  ...
  @PermitAll
  public TreeNode<HcoNodeData> findRoot(
    ...
    Query q =
        em.createNativeQuery(
            "select filter_category_status(0, "
                + destinationIdCsv
                + ", "
                + systemIdCsv
                + ", "
  ...
  @PermitAll
  public List<TreeNode<HcoNodeData>> findChildren(
      HcoNodeType type,
      BigInteger nodeId,
      BigInteger[] destinationIdArray,
      BigInteger categoryId,
    ...
    switch (type) {
      case SYSTEM:
        children =
            findChildrenOfSystem(nodeId, destinationIdArray, regionId, groupId, statusIdArray);
        break;
      case CATEGORY:
        children =
            findChildrenOfCategory(
                nodeId, destinationIdArray, systemIdArray, regionId, groupId, statusIdArray);
        break;
      case COMPONENT:
  ...
  @PermitAll
  private List<Category> findCategoryChildrenList(
      BigInteger categoryId,
      BigInteger[] destinationIdArray,
    ...
    if (!categoryIdList.isEmpty()) {
      Query q =
          em.createNativeQuery(
              "select * from category where category_id in (:categoryIdList) order by weight asc, name asc",
              Category.class);
  ...
  private List<SystemEntity> findSystemChildrenList(
      BigInteger categoryId,
      BigInteger[] destinationIdArray,
      BigInteger[] systemIdArray,
      BigInteger regionId,
    ...
    if (statusIdArray != null && statusIdArray.length > 0) {
      ...
      Predicate p1 = subqueryRoot.<BigInteger>get("statusId").in(Arrays.asList(statusIdArray));
      subquery.where(p1);
      filters.add(cb.in(componentList.get("componentId")).value(subquery));
    }
  ...
  private List<Component> findComponentChildrenList(
      BigInteger systemId,
      BigInteger[] destinationIdArray,
      BigInteger regionId,
      BigInteger groupId,
  ...
  @SuppressWarnings("unchecked")
  private Map<BigInteger, Status> findCategoryStatuses(
      BigInteger categoryId,
      BigInteger[] destinationIdArray,
      BigInteger[] systemIdArray,
      BigInteger regionId,
    ...
    Query q =
        em.createNativeQuery(
            "select * from table(filter_category_status_table("
                + categoryId
                + ", "
                + destinationIdCsv
                + ", "
    ...
    for (Object[] row : results) {
      ...
      statusMap.put(
          BigInteger.valueOf(childCatId.longValue()),
          Status.FROM_ID(BigInteger.valueOf(statusId.longValue())));
    }
  ...
  @SuppressWarnings("unchecked")
  private Map<BigInteger, Status> findSystemStatuses(
      BigInteger[] destinationIdArray,
      BigInteger[] systemIdArray,
      BigInteger regionId,
      BigInteger groupId,
    ...
    Query q =
        em.createNativeQuery(
            "select * from table(filter_system_status_table("
                + destinationIdCsv
                + ", "
                + systemIdCsv
                + ", "
    ...
    for (Object[] row : results) {
      ...
      Number statusId = (Number) row[1];
      statusMap.put(
          BigInteger.valueOf(categoryId.longValue()),
          Status.FROM_ID(BigInteger.valueOf(statusId.longValue())));
    }
  ...
  @SuppressWarnings("unchecked")
  private Map<BigInteger, Status> findComponentStatuses(
      List<Component> components, BigInteger groupId) {
    Map<BigInteger, Status> statusMap = new HashMap<>();
    ...
    if (components != null && !components.isEmpty()) {
      ...
      if (groupId == null) {
        q =
            em.createNativeQuery(
                "select a.component_id, case when a.masked = 'Y' then a.mask_type_id else nvl((select max(b.status_id) from component_signoff b where a.component_id = b.component_id), 1) end as status_id from component a where a.component_id in :componentIdList",
                "ComponentStatus");
      } else {
        q =
            em.createNativeQuery(
                "select a.component_id, case when a.masked = 'Y' then a.mask_type_id else nvl((select b.status_id from component_signoff b where a.component_id = b.component_id and b.group_id = :groupId), 1) end as status_id from component a where a.component_id in :componentIdList",
                "ComponentStatus");
      ...
      for (List<BigInteger> l : plist) {
        ...
        for (Object[] record : componentStatusList) {
          ...
          statusMap.put(
              BigInteger.valueOf(componentIdAsNumber.longValue()),
              Status.FROM_ID(BigInteger.valueOf(statusIdAsNumber.longValue())));
        }
      }
    }
  ...
  @PermitAll
  public List<TreeNode<HcoNodeData>> findChildrenOfCategory(
      BigInteger categoryId,
      BigInteger[] destinationIdArray,
      BigInteger[] systemIdArray,
      BigInteger regionId,
    ...
    List<Category> categoryChildren =
        findCategoryChildrenList(
            categoryId, destinationIdArray, systemIdArray, regionId, groupId, statusIdArray);

    // Find category statuses
    Map<BigInteger, Status> categoryStatuses =
        findCategoryStatuses(
            categoryId, destinationIdArray, systemIdArray, regionId, groupId, statusIdArray);

    // Find system children
    List<SystemEntity> systemChildren =
        findSystemChildrenList(
            categoryId, destinationIdArray, systemIdArray, regionId, groupId, statusIdArray);
    ...
    Map<BigInteger, Status> systemStatuses =
        findSystemStatuses(
            destinationIdArray,
            systemIdArray,
            regionId,
            null,
            statusIdArray); // groupId always = null to avoid pretending status only considers one
    // group
    ...
    if (categoryChildren != null) {
      for (Category category : categoryChildren) {
        boolean lazyChildren =
            ((category.getCategoryList() != null) && !(category.getCategoryList().isEmpty()))
                || ((category.getSystemList() != null) && !(category.getSystemList().isEmpty()));
        HcoNodeData data =
            new HcoNodeData(
                HcoNodeType.CATEGORY,
                category.getCategoryId(),
                category.getName(),
                categoryStatuses.get(category.getCategoryId()),
                lazyChildren);
        TreeNode<HcoNodeData> node = new TreeNode<>(data);
        children.add(node);
      }
    ...
    if (systemChildren != null) {
      for (SystemEntity system : systemChildren) {
        boolean lazyChildren =
            (system.getComponentList() != null) && !(system.getComponentList().isEmpty());
        HcoNodeData data =
            new HcoNodeData(
                HcoNodeType.SYSTEM,
                system.getSystemId(),
                system.getName(),
                systemStatuses.get(system.getSystemId()),
                lazyChildren);
        TreeNode<HcoNodeData> node = new TreeNode<>(data);
        children.add(node);
      }
  ...
  @PermitAll
  public List<TreeNode<HcoNodeData>> findChildrenOfSystem(
      BigInteger systemId,
      BigInteger[] destinationIdArray,
      BigInteger regionId,
      BigInteger groupId,
    ...
    List<Component> componentChildren =
        findComponentChildrenList(systemId, destinationIdArray, regionId, groupId, statusIdArray);

    // Find component statuses
    Map<BigInteger, Status> componentStatuses =
        findComponentStatuses(
            componentChildren,
            null); // groupId always null because we don't want to change component status by
    // pretending component only has one group

    // Create nodes for component children
    if (componentChildren != null) {
      for (Component component : componentChildren) {
        boolean lazyChildren =
            (component.getSystem().getGroupResponsibilityList() != null)
                && !(component.getSystem().getGroupResponsibilityList().isEmpty());
        ...
        HcoNodeData data =
            new HcoNodeData(
                HcoNodeType.COMPONENT, component.getComponentId(), name, status, lazyChildren);
        TreeNode<HcoNodeData> node = new TreeNode<>(data);
        children.add(node);
      }
    }
  ...
  @PermitAll
  public List<TreeNode<HcoNodeData>> findChildrenOfComponent(
      BigInteger componentId, BigInteger groupId) {
    List<TreeNode<HcoNodeData>> children = new ArrayList<>();
    ...
    signoffList =
        signoffFacade.updateAndFetchGroupSignOffList(
            component, null); // always use null groupId so that all groups are always shown!

    for (GroupSignoff signoff : signoffList) {
      HcoNodeData data =
          new HcoNodeData(
              HcoNodeType.GROUP,
              signoff.getGroupResponsibility().getGroup().getGroupId(),
              signoff.getGroupResponsibility().getGroup().getName(),
              signoff.getStatus(),
              false);
      TreeNode<HcoNodeData> node = new TreeNode<>(data);
      children.add(node);
