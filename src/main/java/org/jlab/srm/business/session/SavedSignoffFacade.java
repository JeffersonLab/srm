...
public class SavedSignoffFacade extends AbstractFacade<SavedSignoff> {
    ...
    @EJB
    SavedSignoffTypeFacade typeFacade;
    @EJB
        ...
        for(BigInteger savedSignoffId: savedSignoffIdArray) {
            ...
            if (saved == null) {
                throw new UserFriendlyException(
                        "Saved Signoff with ID "
                                + savedSignoffId + " not found.");
            }
    ...
    public BigInteger add(BigInteger typeId, String signoffName, BigInteger signoffStatusId,
        ...
        if (type == null) {
            throw new UserFriendlyException(
                    "Signoff Type with ID: " + typeId + " not found");
        }
        ...
        if (signoffStatus == null) {
            throw new UserFriendlyException(
                    "Signoff Status with ID: " + signoffStatusId + " not found");
        }
    ...
    private SavedSignoff createSpecial(SavedSignoff signoff) {
        ...
        String sql = "insert into saved_signoff (saved_signoff_id, saved_signoff_type_id, signoff_name, filter_group_id, filter_system_id, filter_region_id, filter_status_id, filter_component_name, signoff_status_id, signoff_comments, weight) "
                + "values (?, ?, ?, ?, ?";
    ...
    public void multipleSignoffs(BigInteger[] savedSignoffIdArray, Date maxLastModified) throws UserFriendlyException {
        ...
        for(BigInteger savedSignoffId: savedSignoffIdArray) {
            ...
            List<Component> componentList
                    = componentFacade.findWithoutSignoff(null,
            ...
            doSignoff(saved.getSignoffStatus().getStatusId(), saved.getSignoffComments(), componentIdList, groupIdList);
        }

    }

    private void doSignoff(BigInteger statusId, String comment, List<BigInteger> componentIdList, List<BigInteger> groupIdList) throws UserFriendlyException {
        ...
        GroupSignoffFacade.SignoffValidateRule validateRule = new GroupSignoffFacade.SignoffValidateRule();
        ...
        signoffFacade.updateSignoff(componentIdList, groupIdList, Status.FROM_ID(statusId), comment, needsAttention, cascadeRule, validateRule);
    }
}
