...
public class ChecklistHistoryFacade extends AbstractFacade<ChecklistHistory> {
  ...
  public void createSpecial(ChecklistHistory checklist) {
    Query q =
        em.createNativeQuery(
            "insert into checklist_history (checklist_history_id, checklist_id, body_html, author, modified_date, modified_username, comments) values (checklist_history_id.nextval, ?, ?, ?, ?, ?, ?)");
  ...
  public int countByChecklistId(BigInteger checklistId) {
    TypedQuery<Long> q =
        em.createQuery(
            "select count(c) from ChecklistHistory c where c.checklist.checklistId = :checklistId",
            Long.class);
  ...
  public List<ChecklistHistory> findByChecklistId(BigInteger checklistId) {
    TypedQuery<ChecklistHistory> q =
        em.createQuery(
            "select c from ChecklistHistory c where c.checklist.checklistId = :checklistId order by c.modifiedDate asc",
            ChecklistHistory.class);
