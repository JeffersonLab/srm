...
public class GroupSignoffHistoryFacade extends AbstractFacade<GroupSignoffHistory> {
  ...
  /**
   * THIS IS A HACK. I can't get ENVERS (Hibernate auditing) to stop trying to version this entity.
   * So I'll just manually do the insertion.
   *
  ...
  public void create(GroupSignoffHistory history) {
    Query q =
        em.createNativeQuery(
            "insert into group_signoff_history (group_signoff_history_id, system_id, group_id, component_id, status_id, modified_username, modified_date, comments, change_type) values (group_signoff_history_id.nextval, ?, ?, ?, ?, ?, ?, ?, ?)");
