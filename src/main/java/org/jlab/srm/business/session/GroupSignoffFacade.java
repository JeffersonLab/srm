...
public class GroupSignoffFacade extends AbstractFacade<GroupSignoff> {
  ...
  private static final Logger LOGGER = Logger.getLogger(GroupSignoffFacade.class.getName());
  ...
  @PermitAll
  public List<GroupSignoff> updateAndFetchGroupSignOffList(
      Component component, BigInteger groupId) {
    ...
    if (component != null && component.getSystem().getGroupResponsibilityList() != null) {
      ...
      for (GroupResponsibility responsibility :
          component.getSystem().getGroupResponsibilityList()) { // Should be ordered by weight
        ...
        if (groupId == null
            || signoff.getGroupResponsibility().getGroup().getGroupId().equals(groupId)) {
          list.add(signoff);
        }
      }
    }
  ...
  @PermitAll
  public void updateSignoff(
      BigInteger componentId,
      BigInteger groupId,
      Status status,
      String comment,
      SignoffCascadeRule cascadeRule,
      SignoffValidateRule validateRule,
      Date signoffDate,
      String username)
      throws UserFriendlyException {
    GroupSignoff signoff = findByComponentIdAndGroupId(componentId, groupId);
    ...
    GroupResponsibility responsibility =
        groupResponsibilityFacade.findBySystemIdAndGroupId(system.getSystemId(), groupId);

    if (responsibility == null) {
      throw new UserFriendlyException("Cannot signoff when there is no responsibility");
    }

    if (validateRule.requiredChecklistPublished) {
      if (responsibility.isChecklistRequired()
          && !responsibility.isPublished()
    ...
    if (validateRule.disallowModifyMask
        && (before.equals(Status.MASKED) || before.equals(Status.MASKED_CC))) {
    ...
    if (type == SignoffChangeType.COMMENT) {
      ...
      if (status.equals(Status.NOT_APPLICABLE)) {
        checkAdminOrGroupLeader(
            ...
            "You must be a group leader or admin to change a signoff status to N/A");

        if (validateRule.atLeastOneNonNa) {
          List<ComponentSignoff> signoffList = componentFacade.findByComponent(componentId);

          boolean atLeastOneNonNa = false;

          for (ComponentSignoff s : signoffList) {
            if (!s.getGroupId().equals(groupId)
                && !s.getStatusId().equals(Status.NOT_APPLICABLE.getStatusId())) {
        ...
        checkAdminOrGroupLeader(
            username,
            responsibility.getGroup(),
            "You must be a group leader or admin to change a signoff status to Ready");

        if (validateRule.twoStepSignoff) {
          if (signoff.getStatus() == null || signoff.getStatus().equals(Status.NOT_READY)) {
            throw new UserFriendlyException(
                "Cannot signoff component \""
                    + component.getName()
                    + "\" for group \""
        ...
        boolean downgrade =
            status.getStatusId().intValue()
                > (signoff.getStatus() == null
                    ? 100
                    : signoff.getStatus().getStatusId().intValue());
        if (downgrade) {
          ...
          if (cascadeRule.cascade) {
            groupResponsibilityFacade.cascadeDowngrade(
                ...
                responsibility.getWeight(),
                username,
                comment,
                cascadeRule.readyCascade,
                cascadeRule.checkedCascade);
          }
        }
      }
    }
  ...
  public Long updateSignoff(
    ...
    if (uniqueGroupList.size() > 1) {
      throw new UserFriendlyException("Cannot signoff for more than one group at a time");
    }
    ...
    for (BigInteger componentId : componentIdList) {
      ...
      updateSignoff(
          componentId, groupId, status, comment, cascadeRule, validateRule, signoffDate, username);
    }
  ...
  public GroupSignoff createSpecial(GroupSignoff signoff) {
    ...
    Query q =
        em.createNativeQuery(
            "insert into group_signoff (group_signoff_id, system_id, group_id, component_id, status_id, modified_username, modified_date, comments, change_type) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
  ...
  public List<SignoffStandIn> findStandInListByComponentList(List<Component> componentList) {
    ...
    LOGGER.log(Level.FINE, "[PERFORMANCE] Fetch Group Signoff List: {0}", (end - start) / 1000.0f);
  ...
  public Map<String, SignoffStandIn> createComponentGroupToSignoffStandInMap(
    ...
    Map<String, SignoffStandIn> signoffMap = new HashMap<>();

    if (signoffList != null) {
      for (SignoffStandIn signoff : signoffList) {
        signoffMap.put(
            signoff.getComponentId().toString() + ";" + signoff.getGroupId().toString(), signoff);
      }
    }
  ...
  @PermitAll
  public void reassignGroupSignoffs(
      Component component, ResponsibleGroup group, SystemEntity newSystem) {
    String sql =
    ...
    sql = "update GroupSignoffHistory set systemId = ?1 where componentId = ?2 and groupId = ?3";
  ...
  public void deleteGroupSignoffs(Component component, SystemEntity system) {
    String sql = "delete from group_signoff where component_id = ?1 and system_id = ?2";
    Query q = em.createNativeQuery(sql);
  ...
  @PermitAll
  public void bulkSignoff(BigInteger systemId, Status status, String comment)
      throws UserFriendlyException {
    List<BigInteger> componentIdList;
    List<BigInteger> groupIdList;
    ...
    for (GroupResponsibility responsibility : responsibilityList) {
      ...
      this.updateSignoff(
          ...
          comment,
          false,
          new SignoffCascadeRule(),
          new SignoffValidateRule());
    }
  }

  private Long createOpsPr(
      List<BigInteger> componentIdList, BigInteger groupId, String comment, String username)
      throws UserFriendlyException {
    ...
    ProblemReport report =
        new ProblemReport(
            ProblemReportType.OPS,
            true,
            system.getSystemId().intValue(),
            groupId.intValue(),
            componentId == null ? null : componentId.intValue());
