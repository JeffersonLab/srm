  ...
  public List<AuditedEntityChange> findEntityChangeList(long revision) {
    Query q =
        em.createNativeQuery(
            "select 'A', component_id, name, revtype from component_aud where rev = ?1 union select 'B', system_id, name, revtype from system_aud where rev = ?2 union select 'C', category_id, name, revtype from category_aud where rev = ?3");
