...
public class GroupResponsibilityFacade extends AbstractFacade<GroupResponsibility> {
  ...
  @PermitAll
  public BigInteger createNew(
      BigInteger systemId, BigInteger groupId, boolean checklistRequired, BigInteger weight)
      throws UserFriendlyException {
    String username = checkAuthenticated();

    if (systemId == null) {
      throw new UserFriendlyException("System ID cannot be null.");
    }

    if (groupId == null) {
      throw new UserFriendlyException("Group ID cannot be null.");
    }
  ...
  @PermitAll
  public void setInitialSystemSignoffs(
      SystemEntity system, ResponsibleGroup group, String comment, String username)
      throws UserFriendlyException {
  ...
  @PermitAll
  public void setInitialComponentSignoffs(
  ...
  private GroupResponsibility createSpecial(GroupResponsibility responsibility) {
    ...
    Query q =
        em.createNativeQuery(
            "insert into group_responsibility (group_responsibility_id, group_id, system_id, weight, checklist_id, checklist_required, published, published_date, published_username) values (?, ?, ?, ?, null, ?, ?, null, null)");
  ...
  @PermitAll
  public void order(BigInteger[] groupResponsibilityIdArray) throws UserFriendlyException {
    String username = checkAuthenticated();
    ...
    for (int i = 0; i < groupResponsibilityIdArray.length; i++) {
      ...
      if (responsibility == null) {
        throw new UserFriendlyException("Group Responsibilty with ID " + id + " not found.");
      }
    ...
    if (systemSet.size() > 1) {
      throw new UserFriendlyException(
          "List of group responsibilties do not all belong to the" + " same system.");
    }
    ...
    if (oldGroupResponsibilityList.size() != newGroupResponsibilityList.size()
        || !oldGroupResponsibilityList.containsAll(newGroupResponsibilityList)) {
      throw new UserFriendlyException(
          "The list of responsibilties to order does" + " not match the list in the database.");
    }
  ...
  @PermitAll
  public GroupResponsibility findBySystemAndGroup(BigInteger systemId, BigInteger groupId)
      throws UserFriendlyException {
    ...
    TypedQuery<GroupResponsibility> q =
        em.createQuery(
  ...
  public void delete(BigInteger groupResponsibilityId) throws UserFriendlyException {
    ...
    if (responsibility == null) {
      throw new UserFriendlyException(
          "Group Responsibilty with ID " + groupResponsibilityId + " not found.");
    }
  ...
  private void removeSpecial(GroupResponsibility responsibility) {
    Query q =
        em.createNativeQuery("delete from group_responsibility where group_responsibility_id = ?");
  ...
  @PermitAll
  public void update(BigInteger groupResponsibilityId, boolean checklistRequired)
      throws UserFriendlyException {
    String username = checkAuthenticated();
    ...
    Category branchRoot = categoryFacade.findBranchRoot(responsibility.getSystem().getCategory());
  ...
  public boolean isSignoffAllowed(BigInteger componentId, BigInteger weight) {
    Query q =
        em.createNativeQuery(
            "select previous_signoff_status(" + componentId + "," + weight + ") from dual");
  ...
  private void doReadyCascade(
      BigInteger componentId,
      BigInteger weight,
      String username,
      String comment,
      BigInteger readyCascade)
      throws UserFriendlyException {
  ...
  private void doCheckedCascade(
      BigInteger componentId,
      BigInteger weight,
      String username,
      String comment,
      BigInteger checkedCascade)
      throws UserFriendlyException {
  ...
  @PermitAll
  public List<GroupResponsibility> filterList(
  ...
  public BigInteger togglePublished(BigInteger groupResponsibilityId) throws UserFriendlyException {
    ...
    if (responsibility == null) {
      throw new UserFriendlyException(
          "Group Responsibility with ID: " + groupResponsibilityId + " not found");
    }
