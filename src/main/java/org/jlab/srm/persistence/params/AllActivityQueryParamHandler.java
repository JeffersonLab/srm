...
public final class AllActivityQueryParamHandler implements QueryParamHandler {
  ...
  public String getSqlWhereClause() {
    ...
    if (params.getDestinationIdArray() != null && params.getDestinationIdArray().length > 0) {
      String subFilter =
          "component_id in (select component_id from component_beam_destination where beam_destination_id in (?";
    ...
    /**
     * Two cases: (1) signoff_activity and group_id is not null (some group did the signoff) (2)
     * inventory_activity and group_id is null (a component/system/category changed)
     *
     * <p>If first case then just check if signoff was done by group (group_id = ?) If second case
     * then check if group is responsible for system in question (note: category changes are IGNORED
     * by group filter as written)
     */
    if (params.getGroupId() != null) {
      filters.add(
          "((group_id is null and system_id in (select system_id from group_responsibility where group_id = ?)) or group_id = ?)");
    }
     ...
     *
     * <p>Two cases: (1) signoff_activity and status_id is not null (some group did the signoff) (2)
     * inventory_activity and status_id is null (a component/system/category changed)
     *
     * <p>If first case then just check if signoffs include status (status_id in ?,...) If second
     * case then need "or status_id is null" so that inventory items are included
     */
     ...
     *
     * <p>This allows us to filter out everything but Not Ready or Checked components for example
     * (so we can ignore activity on components that are now Ready)
     *
     * <p>NOTE: System and Category changes are ignored when the component status filter is used as
     * written
     */
    if (params.getCurrentComponentStatusIdArray() != null
        && params.getCurrentComponentStatusIdArray().length > 0) {
      String subFilter =
          "component_id in (select component_id from component_status_2 where status_id in (?";
     ...
     *
     * <p>This allows us to filter out everything but Not Ready or Checked group signoff for example
     * (so we can ignore activity on signoff that are now Ready)
     *
     * <p>NOTE: A group MUST be specified or the signoff is ambigous
     *
     * <p>NOTE: System and Category changes are ignored when the component status filter is used as
     * written
     */
    if (params.getGroupId() != null
        && params.getCurrentSignoffStatusIdArray() != null
        && params.getCurrentSignoffStatusIdArray().length > 0) {
      String subFilter =
          "component_id in (select component_id from group_signoff where group_id = ? and status_id in (?";
