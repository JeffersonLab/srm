...
public class ComponentAudFacade extends AbstractFacade<ComponentAud> {
    ...
    @PermitAll
    public List<ComponentAud> filterList(BigInteger componentId, BigInteger revisionId, int offset, int max) {
        ...
        List<ComponentAud> entityList = getEntityManager().createQuery(cq).setFirstResult(offset).setMaxResults(max).getResultList();
