...
import org.jlab.hco.persistence.entity.aud.SystemAud;
...
public class SystemAudFacade extends AbstractFacade<SystemAud> {
    ...
    @PermitAll
    public List<SystemAud> filterList(BigInteger systemId, BigInteger revisionId, int offset, int max) {
        ...
        List<SystemAud> entityList = getEntityManager().createQuery(cq).setFirstResult(offset).setMaxResults(max).getResultList();
