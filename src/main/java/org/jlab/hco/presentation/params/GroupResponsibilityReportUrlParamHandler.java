...
import org.jlab.hco.business.params.GroupResponsibilityReportParams;
...
public class GroupResponsibilityReportUrlParamHandler implements
        UrlParamHandler<GroupResponsibilityReportParams> {
    ...
    public GroupResponsibilityReportUrlParamHandler(HttpServletRequest request,
    ...
    public void store(GroupResponsibilityReportParams params) {
        ...
        session.setAttribute("responsibilityReportDestinationId[]",
                params.getDestinationIdArray() == null ? new BigInteger[0] : params.getDestinationIdArray());
        session.setAttribute("responsibilityReportCategoryId[]", new BigInteger[]{params.getCategoryId()});
        ...
        session.setAttribute("responsibilityReportChecklistRequired[]", new Boolean[]{params.isChecklistRequired()});
        session.setAttribute("responsibilityReportChecklistMissing[]", new Boolean[]{params.isChecklistMissing()});
    }
    ...
    public GroupResponsibilityReportParams materialize() {
        ...
        /* Note: We use an array regardless if the parameter is multi-valued because a null array means no page ever set this param before vs empty array or array with null elements means someone set it, but value is empty*/
        HttpSession session = request.getSession(true);
        BigInteger[] destinationIdArray = (BigInteger[]) session.getAttribute("responsibilityReportDestinationId[]");
        BigInteger[] categoryIdArray = (BigInteger[]) session.getAttribute("responsibilityReportCategoryId[]");
        BigInteger[] systemIdArray = (BigInteger[]) session.getAttribute("responsibilityReportSystemId[]");
        BigInteger[] groupIdArray = (BigInteger[]) session.getAttribute("responsibilityReportGroupId[]");
        Boolean[] checklistRequiredArray = (Boolean[]) session.getAttribute("responsibilityReportChecklistRequired[]");
        Boolean[] checklistMissingArray = (Boolean[]) session.getAttribute("responsibilityReportChecklistMissing[]");
     ...
     */
    @Override
    public void redirect(HttpServletResponse response, GroupResponsibilityReportParams params) throws
            IOException {
