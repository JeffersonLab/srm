...
public class ComponentReportUrlParamHandler implements UrlParamHandler<ComponentReportParams> {
    ...
    public ComponentReportUrlParamHandler(HttpServletRequest request, BigInteger[] defaultDestinationIdArray,
                                          BeamDestinationFacade destinationFacade, CategoryFacade categoryFacade,
    ...
    public void store(ComponentReportParams params) {
        ...
        session.setAttribute("componentReportDestinationId[]",
        ...
        session.setAttribute("componentReportComponentName[]", new String[]{params.getComponentName()});
    }
    ...
    public ComponentReportParams materialize() {
        ...
        /* Note: We use an array regardless if the parameter is multi-valued because a null array means no page ever set this param before vs empty array or array with null elements means someone set it, but value is empty*/
        HttpSession session = request.getSession(true);
        BigInteger[] destinationIdArray = (BigInteger[]) session.getAttribute("componentReportDestinationId[]");
        BigInteger[] categoryIdArray = (BigInteger[]) session.getAttribute("componentReportCategoryId[]");
     ...
     */
    @Override
    public void redirect(HttpServletResponse response, ComponentReportParams params) throws
            IOException {
