...
import org.jlab.hco.business.session.SavedSignoffFacade;
 ...
 * @author ryans
 */
@WebServlet(name = "AddSavedDowngrade", urlPatterns
        = {"/setup/ajax/add-saved-signoff"})
public class AddSavedSignoff extends HttpServlet {

    private static final Logger logger = Logger.getLogger(
            AddSavedSignoff.class.getName());

    @EJB
    SavedSignoffFacade savedSignoffFacade;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            BigInteger typeId = ParamConverter.convertBigInteger(
                    request, "typeId");
            String signoffName = request.getParameter("signoffName");
            BigInteger signoffStatusId = ParamConverter.convertBigInteger(
                    request, "signoffStatusId");
            ...
            BigInteger regionId = ParamConverter.convertBigInteger(
                    request, "regionId");
            BigInteger filterStatusId = ParamConverter.convertBigInteger(
                    request, "filterStatusId");
            String componentName = request.getParameter("componentName");

            savedSignoffId = savedSignoffFacade.add(typeId, signoffName, signoffStatusId, comments,
                    systemId, groupId, regionId, filterStatusId, componentName);
        ...
        if (errorReason == null) {
            xml
                    = "<response><span class=\"status\">Success</span><span class=\"saved-signoff-id\">" + savedSignoffId + "</span></response>";
        } else {
