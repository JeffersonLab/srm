 ...
 * @author ryans
 */
@WebServlet(name = "BatchSignoff", urlPatterns = {"/ajax/batch-signoff"})
public class BatchSignoff extends HttpServlet {

    private static final Logger logger = Logger.getLogger(
            BatchSignoff.class.getName());
    @EJB
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            prId = signoffFacade.updateSignoff(componentIdList, groupIdList, Status.FROM_ID(statusId), comment, needsAttention, cascadeRule, validateRule);
        ...
        if (errorReason == null) {
            xml = "<response><span class=\"status\">Success</span><span class=\"pr-id\">" + prId + "</span></response>";
        } else {
