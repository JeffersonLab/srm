...
import org.jlab.hco.business.params.MaskRequestParams;
import org.jlab.hco.business.session.*;
import org.jlab.hco.persistence.entity.*;
import org.jlab.hco.presentation.params.MaskRequestUrlParamHandler;
 ...
 * @author ryans
 */
@WebServlet(name = "MaskRequests", urlPatterns = {"/masks/requests"})
public class Requests extends HttpServlet {

    @EJB
    MaskingRequestFacade recordFacade;
    @EJB
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        MaskRequestUrlParamHandler paramHandler = new MaskRequestUrlParamHandler(request,
        ...
        List<ResponsibleGroup> groupList = groupFacade.findAll(new AbstractFacade.OrderDirective(
                "name"));
        List<MaskingRequest> recordList = recordFacade.find(params.getDestinationIdArray(),
                params.getCategoryId(), params.getSystemId(), params.getRegionId(),
                params.getGroupId(), params.getReason(), params.getStatus(),
                offset, maxPerPage);
        Long totalRecords = recordFacade.count(null, params.getDestinationIdArray(),
                params.getCategoryId(),
                params.getSystemId(), params.getRegionId(), params.getGroupId(), params.getReason(), params.getStatus());
