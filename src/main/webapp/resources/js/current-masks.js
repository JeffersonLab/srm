...
jlab.srm.removeComponentException = function () {
    ...
    $("#unmask-button")
        .height($("#unmask-button").height())
        .width($("#unmask-button").width())
        .empty().append('<div class="button-indicator"></div>');
    $(".dialog-close-button").attr("disabled", "disabled");
    $(".ui-dialog-titlebar button").attr("disabled", "disabled");

    var componentIdArray = $.parseJSON($("#unmask-selected-row-list").attr('data-id-json')),
        doReload = false,
        status = $("#unmask-signoff").val(),
        comment = $("#comment").val();

    var request = jQuery.ajax({
        url: jlab.contextPath + "/ajax/remove-component-exception",
        type: "POST",
        data: {
            'componentId[]': componentIdArray,
            status: status,
    ...
    request.done(function (data) {
        if ($(".status", data).html() !== "Success") {
            alert('Unable to remove component exception: ' + $(".reason", data).html());
        } else {
            doReload = true;
        }
    ...
    request.fail(function (xhr, textStatus) {
        window.console && console.log('Unable to remove component exception: Text Status: ' + textStatus + ', Ready State: ' + xhr.readyState + ', HTTP Status Code: ' + xhr.status);
        alert('Unable to remove component exception; server did not handle request');
    });
...
jlab.srm.openExceptionDialog = function () {
    ...
    $("#component-table .inner-table .selected-row").each(function () {
        var componentName = $(this).closest("tr").find(":nth-child(3) a").text(),
            componentId = $(this).closest("tr").attr("data-component-id"),
            reason = $(this).closest("tr").find(":nth-child(5)").text();
...
jlab.srm.openRemoveExceptionDialog = function () {
    ...
    var $selectedList = $("#unmask-selected-row-list");
    ...
    var count = $("#selected-count").text() * 1;
    var componentStr = (count === 1) ? ' Component' : ' Components';
    $("#unmask-exception-dialog-component-count").text(count + componentStr);
